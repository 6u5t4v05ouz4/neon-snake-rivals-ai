...
function normalizeAccountKeys(tx: any): PublicKey[] {
    const msg = tx.transaction.message;

    // Legacy transactions: accountKeys is a PublicKey[]
    if (Array.isArray(msg.accountKeys) && msg.accountKeys.length > 0 && msg.accountKeys[0] instanceof PublicKey) {
...
function normalizeInstructions(tx: any): { programIdIndex: number; accountIndices: number[]; data: string }[] {
    const msg = tx.transaction.message;

    // V0 compiled instructions
    if (msg.compiledInstructions?.length > 0) {
        return msg.compiledInstructions.map((ci: any) => ({
            programIdIndex: ci.programIdIndex,
            accountIndices: ci.accountIndices,
            data: ci.data,
        }));
    }
...
export async function verifyBetTransaction(params: BetVerificationParams): Promise<VerificationResult> {
    ...
    const msg = tx.transaction.message;
    const rawCI = msg.compiledInstructions?.[0];
    console.log("txVerification DEBUG:", {
        compiledInstructionKeys: rawCI ? Object.keys(rawCI) : 'none',
        rawCI: rawCI ? JSON.stringify(rawCI).slice(0, 500) : 'none',
    ...
    for (const ix of allInstructions) {
        ...
        const poolKey = accountKeys[ix.accountIndices[0]];
        const userKey = accountKeys[ix.accountIndices[2]];
    ...
    console.log("txVerification: no placeBet found.", {
        accountKeysCount: accountKeys.length,
        instructionsCount: allInstructions.length,
        programs: allInstructions.map(ix => accountKeys[ix.programIdIndex]?.toBase58?.() ?? "unknown"),
    });
...
export async function verifyClaimTransaction(params: ClaimVerificationParams): Promise<VerificationResult> {
    ...
    for (const ix of allInstructions) {
        ...
        const poolKey = accountKeys[ix.accountIndices[0]];
        const userKey = accountKeys[ix.accountIndices[1]];
